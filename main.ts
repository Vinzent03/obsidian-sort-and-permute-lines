...
export default class SortAndPermuteLinesPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: () =>
        this.sortListRecursively(true, (a: ListPart, b: ListPart) => {
          if (
            checkboxRegex.test(a.title.source) &&
            !checkboxRegex.test(b.title.source)
          )
            return 1;
          if (
            !checkboxRegex.test(a.title.source) &&
  ...
  sortAlphabetically(fromCurrentList = false, ignoreCheckboxes = true) {
    ...
    let sortFunc = (a: MyLine, b: MyLine) =>
  ...
  getLineCacheFromLine(
    line: number,
    linesCache: ListItemCache[],
  ): ListItemCache | undefined {
    return linesCache.find(
      (cacheItem) => cacheItem.position.start.line === line,
    );
  }
  ...
  getSortedListParts(
    ...
    while (
      startListCache.parent <
        this.getLineCacheFromLine(index + 1, linesCache)?.parent ||
      (startListCache.parent < 0 &&
        this.getLineCacheFromLine(index + 1, linesCache)?.parent >= 0)
    ) {
      index++;
  ...
  getLines(fromCurrentList = false, ignoreCheckboxes = true): MyLine[] {
    ...
    const myLines = lines.map((line, index) => {
      ...
      links.forEach((e) => {
        ...
        myLine.formatted = myLine.formatted.replace(
    ...
    headings?.map(
      (heading) =>
        (myLines[heading.position.start.line].headingLevel = heading.level),
    );
  ...
  setLines(lines: MyLine[], fromCurrentList: boolean = false) {
    ...
    if (res.start != res.end) {
      editor.replaceRange(
        ...
        { line: res.start, ch: 0 },
        { line: res.end, ch: res.endLineLength },
      );
    } else {
